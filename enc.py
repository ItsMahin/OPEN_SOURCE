...
\033[1;92m[Programmed By MEHEDI HASAN]
\033[1;92m[Facebook : Md Mehu
\033[1;92m[Warning : I am not responsible for miss Using ]
...
def ige():
    ...
    os.system("xdg-open https://www.facebook.com/theorygg1")
    time.sleep(7)
    input("\n [BACK]")
    main()           
...
#def about():
    ...
    print(yellow+'║'+cyan+'[ Author ]    '+purple+' :[MEHEDI] ')
    print(yellow+'║'+cyan+'[ Github ]    '+purple+' :[https://github.com/H4X-GG] ')
    print('\n[01] Main Menu')
    print('[02] Exit')
...
def obfuscate(VARIABLE_NAME, file_content):
    ...
    for _ in range(int(len(b64_content) / OFFSET) + 1):
        ...
        index += OFFSET
    code += f'exec(__import__("\\x62\\x61\\x73\\x65\\x36\\x34").b64decode({VARIABLE_NAME}.encode("MEHEDI/H4X/GG/BAN")).decode("MEHEDI/H4X/GG/BAN"))'
    return code
...
def encryptsh():
    ...
    with open(".temp",'r') as temp_f, open(out_file,'w') as out_f:
        ...
        out_f.write("# Encrypted by MEHEDI ( H4X )\n# Github- https://github.com/H4X-GG\n\n"+filedata)
    os.remove(".temp")
...
def decryptsh():
    ...
    with open(".temp2",'r') as temp_f2, open(out_file,'w') as out_f:
        ...
        out_f.write("# Encrypted by H4X ( MEHEDI )\n# Github- https://github.com/H4X-GG\n\n"+filedata)
    os.remove(".temp2")
    sprint(success + out_file + " saved in "+pwd)
    mover(out_file)
